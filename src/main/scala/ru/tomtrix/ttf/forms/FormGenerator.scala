...
def generateForm(parent: Shell)(f: (Shell) => Seq[Any])(callback: Seq[Any] => Unit) {
  val shell = new Shell(Display getDefault)
  val elements = f(shell)
/*val (x, y) = rearrangeAndGetXY(elements)
  w TTFButton(shell, SWT.NONE) {text=ttf"buttons.ok"; left=max(STD_MARGIN, x-STD_MARGIN-2*BTN_WIDTH); top=y; onClick = { e =>
