...
import ru.tomtrix.ttf.controls.{TTFAbstractControl, SingleParameters}
...
def generateForm(parent: Shell, ttfcontrols: Shell => Seq[TTFAbstractControl]) (onOkClick: Seq[String] => Unit) {
  val shell = new Shell(Display getDefault)
  val controls = ttfcontrols(shell)
  ...
  createButton(new SingleParameters(shell, max(STD_MARGIN, x-STD_MARGIN-2*STD_WIDTH), y, ttf"buttons.ok")) { e =>
    onOkClick(controls map {_.getResult})
    shell close()
  }
